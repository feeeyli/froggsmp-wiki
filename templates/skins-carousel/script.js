  ...
  Array.from($slider.find("li"))
    ...
    .forEach(function ($item, i) {
      ...
      if (i === 2) {
        ...
        $slider
          .find(
            '.skins-carousel__name[data-index="' + $($item).data("index") + '"]'
          )
          .addClass("--active");

        $slider
          .find(".skins-carousel__description.--active")
          .removeClass("--active");
        $slider
          .find(
            '.skins-carousel__description[data-index="' +
              $($item).data("index") +
              '"]'
          )
          .addClass("--active");
...
function next($slider) {
  ...
  if ($slider.find(".slider__item--pos-prev")) {
    $slider.find(".slider__item--pos-prev")[0].className =
      "slider__item--outside";
  }

  $slider
    .find(".skins-carousel__slider")[0]
    .insertAdjacentElement("beforeend", $slider.find("li:first-child")[0]);
...
function prev($slider) {
  ...
  if ($slider.find(".slider__item--pos-next")) {
    $slider.find(".slider__item--pos-next")[0].className =
      "slider__item--outside";
  }

  $slider
    .find(".skins-carousel__slider")[0]
    .insertAdjacentElement("afterbegin", $slider.find("li:last-child")[0]);
